...
# make sure our two JSON control files exist
    ...
    def walk(parts):
        for p in parts or []:
            if p.get("mimeType")=="text/plain" and p.get("body",{}).get("data"):
                return urlsafe_b64decode(p["body"]["data"])\
...
# ─── ACCESS CONTROL ──────────────────────────────────────────────────────────
# load admin IDs & unlocked groups
...
def is_authorized(chat_id: int, user_id: int, chat_type: str) -> bool:
    # private chats: only explicit admins
    if chat_type == "private":
        return is_admin(user_id)

    # group already unlocked?
    if chat_id in allowed_groups:
        return True

    # first admin message in a group unlocks it
    if is_admin(user_id):
...
async def cmd_rights(msg: Message):
    permitted = is_authorized(msg.chat.id, msg.from_user.id, msg.chat.type)
    if permitted:
...
async def cmd_start(msg: Message):
    if not is_admin(msg.from_user.id):
        return await msg.reply("❌ Only admins can authorize email.")
...
# ─── COMMAND: /whitelist (any authorized user) ───────────────────────────────
...
async def cmd_whitelist(msg: Message):
    ...
    text  = "Whitelist is empty." if not lines else "\n".join(f"{i+1}. {e}"
...
async def cb_wl_add(q: CallbackQuery):
    ...
    await q.answer("Send the email to ADD (e.g. user@domain.com or *@domain.com).")
...
async def cb_wl_remove(q: CallbackQuery):
    ...
    wl = open(WHITELIST_FILE, encoding="utf-8").read().splitlines()
    if not wl:
...
async def on_remove(m: Message):
    ...
    await m.reply(f"Remove `{html.escape(lines[i])}`?",
...
async def cb_confirm_remove(q: CallbackQuery):
    ...
    await q.message.edit_text(f"❌ Removed `{html.escape(removed)}`.",
                              parse_mode="Markdown")
...
async def cb_mark_read(q: CallbackQuery):
    ...
    info = load_json(UNREAD_STORE_FILE, {}).pop(gid, None)
    save_json(UNREAD_STORE_FILE, load_json(UNREAD_STORE_FILE, {}))
